...
class BillBankModelTest(TestCase):
    ...
    def test_get_active_list(self):
        ...
        self.assertEqual(active_list.first(), self.bill_bank)
...
class BillBankFormTest(TestCase):
    ...
    def setUpTestData(cls):
        cls.bank = Bank.objects.create(name='りそな銀行', bank_code='0010')
        cls.branch = BankBranch.objects.create(bank=cls.bank, name='大阪支店', branch_code='100')
...
class BillBankViewTest(TestCase):
    ...
    def test_bill_bank_create_view_post(self):
        """会社銀行作成ビュー（POST）のテスト"""
        bank2 = Bank.objects.create(name='新しい銀行', bank_code='0020')
        BankBranch.objects.create(bank=bank2, name='新しい支店', branch_code='200')
        form_data = {
            'bank_code': '0020',
            'branch_code': '200',
            'account_type': 'current',
