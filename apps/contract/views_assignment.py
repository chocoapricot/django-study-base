...
def assignment_employment_conditions_text(request, assignment_pk):
    """
    契約アサインの就業条件明示書をテキスト形式で表示する
    """
    from .utils import format_client_user_info, format_company_user_info, format_worktime_pattern
    ...
    assignment = get_object_or_404(
        ContractAssignment.objects.select_related(
            ...
            'staff_contract__staff',
            'client_contract__haken_info__haken_office',
            'client_contract__haken_info__haken_unit',
            'client_contract__haken_info__commander',
            'client_contract__haken_info__complaint_officer_client'
        ),
        pk=assignment_pk
    ...
    # 派遣契約かどうかチェック
    if assignment.client_contract.client_contract_type_code != Constants.CLIENT_CONTRACT_TYPE.DISPATCH:
        messages.error(request, 'この契約は派遣契約ではないため、就業条件明示書を表示できません。')
        return redirect('contract:contract_assignment_detail', assignment_pk=assignment_pk)
    ...
    if hasattr(client_contract, 'haken_info') and client_contract.haken_info:
        h_info = client_contract.haken_info
        
        if h_info.commander:
            items.append({"title": "派遣先指揮命令者", "text": format_client_user_info(h_info.commander, default_text="-")})
            
        if h_info.responsible_person_company:
            comp_resp_title = "製造業務専門派遣元責任者" if (client_contract.job_category and client_contract.job_category.is_manufacturing_dispatch) else "派遣元責任者"
            items.append({"title": comp_resp_title, "text": format_company_user_info(h_info.responsible_person_company, with_phone=True, default_text="-")})

        if h_info.responsible_person_client:
            client_resp_title = "製造業務専門派遣先責任者" if (client_contract.job_category and client_contract.job_category.is_manufacturing_dispatch) else "派遣先責任者"
            items.append({"title": client_resp_title, "text": format_client_user_info(h_info.responsible_person_client, with_phone=True, default_text="-")})

        if h_info.complaint_officer_company:
            items.append({"title": "派遣元苦情申出先", "text": format_company_user_info(h_info.complaint_officer_company, with_phone=True, default_text="-")})
            
        if h_info.complaint_officer_client:
            items.append({"title": "派遣先苦情申出先", "text": format_client_user_info(h_info.complaint_officer_client, with_phone=True, default_text="-")})
...
def assignment_ai_check(request, assignment_pk):
    ...
    if request.method == 'POST':
        default_prompt = "あなたは日本の労働法に詳しい社労士です。以下の就業条件明示書の内容を確認し、労働者派遣法等の法的な観点や記載漏れのリスク、矛盾点があれば指摘してください。問題がなければその旨を伝えてください。\n\n【契約内容】\n{{contract_text}}"
        ai_response, error_message = run_ai_check('GEMINI_PROMPT_TEMPLATE_ASSIGNMENT', full_contract_text, default_prompt)
    
    context = {
        'page_title': '就業条件明示書(AI確認)',
        'header_title': f'就業条件明示書(AI確認) - {assignment.client_contract.contract_name} / {assignment.staff_contract.staff.name}',
        'info_message': '生成AIによる就業条件明示書の内容チェックを行います。',
        'back_url': reverse('contract:contract_assignment_detail', args=[assignment_pk]),
        'is_draft': assignment.staff_contract.contract_status in ['10', '20'],
        'assignment': assignment,
