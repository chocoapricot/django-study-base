...
def download_staff_contract_pdf(request, pk):
    ...
    # セキュリティチェック: 管理者またはログインユーザー自身の契約のPDFのみダウンロード可能
    is_admin = request.user.is_superuser or request.user.has_perm('contract.view_staffcontract')
    if not is_admin and print_history.staff_contract.staff.email != request.user.email:
        raise Http404("このPDFにアクセスする権限がありません")
...
@login_required
@permission_required('contract.view_staffcontract', raise_exception=True)
def staff_contract_ai_check(request, pk):
    """
    ...
    if request.method == 'POST':
        ...
        ai_response, error_message = run_ai_check('PROMPT_TEMPLATE_STAFF', full_contract_text, default_prompt)
            ...
            if flag.company_user and flag.company_user.email:
                ...
                try:
                    ...
                    Notification.objects.create(
                        user=user,
                        title=f'スタッフ契約フラッグが登録されました',
                        message=f'スタッフ契約「{contract.contract_name}」にフラッグ「{flag.flag_status.name}」が登録されました。',
                        notification_type='general',
                        link_url=reverse('contract:staff_contract_detail', kwargs={'pk': contract.pk}),
            ...
            if has_changes:
                ...
                if updated_flag.company_user and updated_flag.company_user.email:
                    ...
                    try:
                        ...
                        Notification.objects.create(
                            user=user,
                            title=f'スタッフ契約フラッグが更新されました',
                            message=f'スタッフ契約「{contract.contract_name}」のフラッグ「{updated_flag.flag_status.name}」が更新されました。',
                            notification_type='general',
                            link_url=reverse('contract:staff_contract_detail', kwargs={'pk': contract.pk}),
