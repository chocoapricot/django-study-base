...
def staff_payroll_detail(request, staff_pk):
    ...
    try:
        payroll = StaffPayroll.objects.get(tenant_id=staff.tenant_id, staff=staff)
...
def staff_payroll_create(request, staff_pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            payroll.staff = staff
            payroll.tenant_id = staff.tenant_id
            payroll.save()
...
def staff_payroll_edit(request, staff_pk):
    """スタッフの給与情報編集"""
    staff = get_object_or_404(get_staff_queryset(request), pk=staff_pk)
    payroll = get_object_or_404(get_filtered_queryset(request, StaffPayroll), staff=staff)
