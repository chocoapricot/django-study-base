...
### 概要
このシステムは、各ページのバリデーションロジックや自動処理について、モーダルインターフェースを通じてユーザーに情報を提供します。

### 実装ルール
- **コーヒーカップアイコン**: ヘッダーに表示してページロジック情報の有無を示す
...
### 技術的実装
1. **ベーステンプレート設定**: `templates/common/_base.html`にモーダル構造とJavaScriptを追加
2. **ページ固有ロジック**: 各フォームテンプレートのJavaScriptでロジック情報を追加
3. **アイコン状態管理**: `data-has-logic`属性でアイコン状態を制御
4. **コンテンツ構造**: 一貫したスタイリングでカードベースレイアウトを使用

### メンテナンスルール
**重要**: フォームでバリデーションロジックや自動処理を追加・修正した場合は、必ず対応するロジック情報表示も更新してください。これにより、ユーザーがページの動作について正確な情報を得られます。
