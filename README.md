...
わたしがDjangoを学習するためのプロジェクトです。スタッフ管理とクライアント管理機能を持つビジネス管理システムを通じて、Djangoの基本的な機能から応用まで学習するために作成したプロジェクトです。Django 5.2.8をベースに構築され、日本語環境に最適化されています。

> **注意**: このプロジェクトは個人の学習目的で作成されており、実際のビジネス用途での使用はできません。個人利用に限定されます。

## 📷 システム画面イメージ

![ダッシュボード](_sample_data/screenshot/02_dashboard.png)

その他の画面イメージについては、[こちら](_sample_data/screenshot/screen_images.md)を参照してください。
...
- **勤怠管理**: スタッフの日次・月次勤怠管理機能を提供し、労働時間、残業時間、休日労働などの自動計算、および承認ワークフローをサポートします。
...
| `apps_master_client_contact_type` | `ClientContactType` | クライアント連絡種別マスタ |
| `apps_master_client_regist_status` | `ClientRegistStatus` | クライアント登録状況 |
| `apps_master_client_tag` | `ClientTag` | クライアントタグマスタ |
...
| `apps_master_staff_contact_type` | `StaffContactType` | スタッフ連絡種別マスタ |
| `apps_master_staff_regist_status` | `StaffRegistStatus` | スタッフ登録状況 |
| `apps_master_staff_tag` | `StaffTag` | スタッフタグマスタ |
| `apps_master_time_punch` | `TimePunch` | タイムパンチマスター |
...
| `apps_system_notification` | `Notification` | 通知 |
...
このプロジェクトは学習目的で作成されており、個人利用に限定されます。商用利用は禁止します。
