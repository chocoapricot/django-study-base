...
## 2025-08-04
- **部署管理機能の強化**:
    - 部署モデルに有効期間（`valid_from`, `valid_to`）フィールドを追加し、期間による有効性チェックを実装。
    - 有効期間の重複チェックを追加し、データの一貫性を向上。
    - 管理画面（Django Admin）で有効期間によるフィルタリングと現在の有効性の表示を追加。
    - 部署一覧表示に有効期間と現在の有効性を示すバッジを追加。
- **テストの追加**:
    - 部署の有効期間に関するテスト（期間重複、有効性チェック、有効な部署の取得）を追加。
