...
### Added
- 勤怠登録状況管理画面（`/kintai/status/management/`）を追加しました。
    - 契約ごとに、打刻データ（StaffTimerecord）、打刻承認（StaffTimerecordApproval）、スタッフ勤怠（StaffTimesheet/Timecard）、クライアント勤怠（ClientTimesheet/Timecard）の登録・承認状況を一覧で確認できます。
    - 各段階のステータスを色分けして表示し、進捗状況を視覚的に把握しやすくしました。
    - 締め処理のために、どの契約がどこまで進んでいるかを一目で確認できる機能です。
    - 総合ステータスとして、「未着手」「打刻あり」「進行中」「打刻承認済」「スタッフ勤怠承認済」「完了」の6段階で表示します。

## 2026-02-16

### Changed
- 403(アクセス拒否)および404(ページが見つかりません)エラーページのデザインを修正しました。
    - アイコンサイズ、フォントサイズを縮小し、他の画面との統一感を向上させました（アイコン: 5rem→3rem, タイトル: display-4→display-6）。
    - 圧迫感を軽減し、より落ち着いたデザインに変更しました。
    - ボタンのサイズを標準的な `btn-sm` に変更し、他の画面と統一しました。

## 2026-02-15

### Added
- クライアント勤怠の検索画面のURLを `/kintai/client/contract/search/` に戻しました。
- クライアント勤怠の検索（契約一覧）画面のレイアウトを改善しました。
    - スタッフ側の画面と同様に、「社員番号」「雇用形態（バッジ）」「契約期間」の列を追加しました。
    - カラム構成の整理に伴い、「有給日数」列を削除しました。
- `ContractAssignment` モデルに `assignment_period_display` プロパティを追加しました。
- メニューに「クライアント勤怠」を追加しました。
    - Company側の管理者向け機能として、クライアント契約に関連する勤怠情報を検索・閲覧できるメニュー（`/kintai/client/contract/search/`）を追加しました。
- 「クライアント勤怠」一覧画面のデザインを「スタッフ検索」画面と統一しました。
    - カードスタイル、検索フォーム、テーブルのレイアウトを `kintai/staff_search.html` に合わせるよう更新しました。
- 「クライアント勤怠」の操作ボタンを「新規作成」から「勤怠入力」に変更し、直接カレンダー形式の入力画面に遷移するように改善しました。
- クライアント勤怠に個別の日次勤怠登録・編集・削除機能を追加しました。
    - スタッフ勤怠と同様の個別登録フォーム（`client_timecard_form.html`）を実装しました。
    - プレビュー画面や一覧画面から、個別の勤怠データの追加・編集・削除が可能になりました。
- クライアント初回勤怠入力時（プレビューから遷移時）の操作フローを改善しました。
    - リダイレクトを挟まず、直接「初回カレンダー入力」画面に遷移するように変更しました。
    - 初回入力画面においても、基本情報ヘッダーを表示しつつ、既に選択済みの契約プルダウンを非表示にすることで操作性を向上させました。

### Changed
- HOME画面の「業務サマリ」および「登録状況」セクションにおいて、要素の高さが揃うように改善しました。
    - CSS`stats-card` クラスに `height: 100%` を追加し、グリッド内でカードの高さが統一されるようにしました。
    - `_stats_card.html` テンプレートを修正し、リンク（`<a>`）が含まれる場合でも高さが正しく継承されるよう `h-100 d-block` クラスを追加しました。
- スタッフ給与関連情報の登録・編集画面において、「前職会社名」と「前職退職日」にヘルプアイコンを追加しました。
    - アイコンにマウスを合わせると、「雇用保険被保険者番号」が不明な場合にスタッフから収集が必要である旨のメッセージが表示されます。
- マイプロフィールの給与情報編集画面において、「雇用保険被保険者番号」、「前職会社名」、「前職退職日」にヘルプアイコンを追加しました。
    - 各項目に、入力が必要なケースや不明な場合の対処法を説明するヘルプテキストを表示するようにしました。
